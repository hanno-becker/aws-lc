...
if [ "$CHUNKS" = "" ]; then
    CHUNKS="16,128,256,512,1350,2048,4096,8192,16384,32768,65536,131072,262144,524288,1048576"
    echo "Environment variable CHUNKS not set. Defaulting to CHUNKS=$CHUNKS."
fi
...
TIMEOUT=20 # Run tests for 20 seconds -- they often hang upon a bug
...
test_variant() {
    if [ $VERBOSE -eq 1 ]; then
        timeout --foreground $TIMEOUT ${AWS_LC_BASE}/${BUILD_DIR}/crypto/crypto_test --gtest_filter="*AES*GCM*"
    else
        timeout --foreground $TIMEOUT ${AWS_LC_BASE}/${BUILD_DIR}/crypto/crypto_test --gtest_filter="*AES*GCM*" > /dev/null
    fi
}

bench_variant() {
    NUM_CHUNKS=$(echo "$CHUNKS" | tr ',' '\n' | wc -l)
    GREP_LIMIT=$((NUM_CHUNKS + 1))
    grep -m $GREP_LIMIT $ENCDEC <(AWSLC_USE_SLOTHY=$AWSLC_USE_SLOTHY stdbuf -oL ${AWS_LC_BASE}/${BUILD_DIR}/tool/bssl speed -filter "AES-${SZ}-GCM" -chunks "$CHUNKS")
}

do_variant() {
    if [ "$1" = "awslc" ]; then
        echo "* Testing variant: awslc (default)"
        export AWSLC_USE_SLOTHY=0
    else
        echo "* Testing variant: ${OPT_STR}/${ENCDEC}/${SZ}_$1"
        printf " - Copy... "
        set_variant $1
        printf "OK!\n"
        export AWSLC_USE_SLOTHY=1
    fi
    printf " - Build... "
    build_variant
    printf "OK!\n"
